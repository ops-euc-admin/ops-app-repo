/**
 * 2_convert-to-dify-format.js
 * * Notionから取得したCSV文字列を、Difyナレッジベース用の「汎用ドキュメント」形式に変換します。
 * * [変更点] Q&A形式を廃止し、ページの全情報を一つのテキストブロックにまとめる方式に変更しました。
 * * [変更点] 不安定なAI要約機能を削除しました。
 * * [修正] 各セクション間の不要な改行を削除し、意図しないチャンク分割を防止するようにしました。
 * * 実行方法:
 * node 2_convert-to-dify-format.js <入力CSVファイルパス>
 */
const fs = require('fs');
...
if (require.main === module) {
    ...
    if (!inputFilePath) {
        console.error('❌ 使用法: node 2_convert-to-dify-format.js <入力CSVファイル>');
        process.exit(1);
    }
