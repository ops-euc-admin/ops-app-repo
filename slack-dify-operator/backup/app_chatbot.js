// Slack × Dify Bot - Streaming/Blocking 切り替え対応版

// === モード切り替え ===
const USE_STREAMING = true; // ← ここを false にすると blocking モードに切り替わります
...
// Blocking モード
async function queryDifyAgentBlocking(query, userId, conversationId = null) {
  const payload = {
    inputs: {},
    query,
  ...
  try {
    ...
    return {
      ...
      fullAnswer: data.answer || 'Difyからの応答がありません。'
    };
  } catch (err) {
    console.error('Dify blocking fetch error:', err);
    return { conversation_id: null, fullAnswer: 'Difyとの通信に失敗しました。' };
  }
}
...
app.event('app_mention', async ({ event, client }) => {
  ...
  if (USE_STREAMING) {
    ...
    // Blocking モード
    const { conversation_id, fullAnswer } = await queryDifyAgentBlocking(cleanQuery, userId, previousConversationId);
    await client.chat.postMessage({
      channel: event.channel,
      thread_ts: event.ts,
      text: `<@${event.user}> ${cleanForSlack(fullAnswer)}`
    });
